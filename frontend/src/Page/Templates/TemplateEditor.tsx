						...
						{selectedLayout && availableElements.includes(selectedElement!) && <div className='TemplateEditor__editorInspector'>
							...
							{selectedPhotoKey && selectedPhoto && <details className='TemplateEditor__settingsGroup TemplateEditor__inspectorPanel' open hidden={activeInspectorTab !== 'content'}>
								...
								{selectedPhoto.mode === 'photo' ? <>
									...
									<div className='TemplateEditor__row'><label>Яркость: {selectedPhoto.brightness}%<input type='range' min='50' max='150' value={selectedPhoto.brightness} onChange={e => updatePhoto(selectedPhotoKey, { brightness: Number(e.target.value) })} /></label><label>Контраст: {selectedPhoto.contrast}%<input type='range' min='50' max='150' value={selectedPhoto.contrast} onChange={e => updatePhoto(selectedPhotoKey, { contrast: Number(e.target.value) })} /></label></div>

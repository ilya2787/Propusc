...
const LayoutCardPass: FC<TypeProps> = ({
	...
	const backgroundSource = storedBackground === null
		? ''
		: storedBackground === 'builtin:flag' ? FlagBackground
			: storedBackground === 'builtin:emblem' ? EmblemBackground
				: resolveServerImageUrl(storedBackground) || (activeTemplate.design.background === 'emblem' ? EmblemBackground : FlagBackground)
	...
	const fittedDirectorField = (key: 'passDirectorPost' | 'passDirectorName', text: string) => {
		const props = editable(key)
		const fontSize = sizes[key] * dimensions.contentScale
	...
	return (
		...
		>
			<div
				style={{ ...cardStyle, width: dimensions.widthPx, height: dimensions.heightPx, boxSizing: 'border-box', border: activeTemplate.design.borderColor ? `${dimensions.contentScale}px solid ${activeTemplate.design.borderColor}` : undefined }}
				className={
			...
			>
				...
				{activeTemplate.design.showDirector && <>
					{fittedDirectorField('passDirectorPost', PostDirector)}
					{fittedDirectorField('passDirectorName', NameDirector)}
				</>}
